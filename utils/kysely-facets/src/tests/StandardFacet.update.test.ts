...
describe("update()", () => {
  it("updates without returning", async () => {
    const insertedUser = await passThruUserFacet.insertOne(USERS[0], ["id"]);
    const updateValues = { email: "new.email@xyz.pdq" };

    const result = await passThruUserFacet.update(
      { id: insertedUser.id },
      updateValues
    );
    expect(result).toBeUndefined();

    const readUser = await passThruUserFacet.selectOne([
      "id",
      "=",
      insertedUser.id,
    ]);
  ...
  it("updates returning update count", async () => {
    const insertedUser0 = await passThruUserFacet.insertOne(USERS[0], ["id"]);
    await passThruUserFacet.insertOne(USERS[1]);
    await passThruUserFacet.insertOne(USERS[2]);

    const updateValues = { email: "new.email@xyz.pdq" };
    const updateCount1 = await passThruUserFacet.update(
      { id: insertedUser0.id },
      updateValues,
      []
    );
    ...
    const updateCount2 = await passThruUserFacet.update(
      { name: "Sue" },
      updateValues,
      []
    );
    ...
    const readUsers = await passThruUserFacet.selectMany(["name", "=", "Sue"]);
  ...
  it("updates returning indicated columns", async () => {
    await passThruUserFacet.insertOne(USERS[0]);
    const insertedUser = await passThruUserFacet.insertOne(USERS[1], ["id"]);
    await passThruUserFacet.insertOne(USERS[2]);

    // Verify that update performs the correct change on the correct row.
    const updateValues1 = { email: "new.email@xyz.pdq" };
    const updatedUsers1 = await passThruUserFacet.update(
      { id: insertedUser.id },
      updateValues1,
      ["name"]
    );
    expect(updatedUsers1).toEqual([{ name: USERS[1].name }]);
    let readUser = await passThruUserFacet.selectOne([
      "id",
      "=",
      insertedUser.id,
    ]);
    ...
    const updatedUsers2 = await passThruUserFacet.update(
    ...
    readUser = await passThruUserFacet.selectOne(["id", "=", insertedUser.id]);
    ...
    const updatedUsers3 = await passThruUserFacet.update(
      { name: "Sue" },
      updateValues3,
      ["handle"]
    );
  ...
  it("updates returning all columns", async () => {
    const insertedUsers = await passThruUserFacet.insertMany(USERS, ["id"]);

    const updateValues = { email: "new.email@xyz.pdq" };
    const updatedUsers = await passThruUserFacet.update(
      { name: "Sue" },
      updateValues,
      ["*"]
    );
  ...
  it("updates all rows when no filter is given", async () => {
    ...
    const updateValues = { email: "new.email@xyz.pdq" };
    const updatedUsers = await passThruUserFacet.update({}, updateValues, [
      "handle",
    ]);
  ...
  it("updates rows indicated by a binary operator", async () => {
    ...
    const updateValues = { email: "new.email@xyz.pdq" };
    const updateCount = await passThruUserFacet.update(
  ...
  ignore("detects update() type errors", async () => {
    ...
    // @ts-expect-error - table must have all filter fields
    passThruUserFacet.update(["notThere", "=", "foo"], {
      email: "abc@def.ghi",
    });
    // @ts-expect-error - update must only have table columns
    passThruUserFacet.update({ id: 32 }, { notThere: "xyz@pdq.xyz" });
    // @ts-expect-error - returning argument can't be a string
    passThruUserFacet.update({ id: 32 }, USERS[0], "id");
    // @ts-expect-error - returning argument can't be a string
    passThruUserFacet.update({ id: 32 }, USERS[0], "*");
    // @ts-expect-error - returning arguments must be valid column names
    passThruUserFacet.update({ id: 32 }, USERS[0], [""]);
    // @ts-expect-error - returning arguments must be valid column names
    passThruUserFacet.update({ id: 32 }, USERS[0], ["notThere"]);
    // @ts-expect-error - returning arguments must be valid column names
    passThruUserFacet.update({ id: 32 }, USERS[0], ["notThere", "*"]);
    // @ts-expect-error - doesn't allow plain string expression filters
    passThruUserFacet.update("name = 'John Doe'", USERS[0]);
    // @ts-expect-error - only requested columns are accessible
    (await passThruUserFacet.update({ id: 32 }, USERS[0], ["id"]))[0].name;
  });
});
