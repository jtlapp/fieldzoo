...
describe("facet for table with unique ID", () => {
  it("inserts, selects, updates, and deletes objects by ID", async () => {
    // Add users
    const id0 = (await explicitIdFacet.insertOne(USERS[0])).id;
    const id1 = (await explicitIdFacet.insertOne(USERS[1])).id;
  ...
  it("provides a default ID of 'id'", async () => {
    ...
    await defaultIdFacet.insertOne(USERS[0]);
    const id1 = (await defaultIdFacet.insertOne(USERS[1])).id;
  ...
  it("allows for returning other columns with the ID", async () => {
    ...
    const insertReturn1 = await idAndHandleFacet.insertOne(USERS[0]);
    expect(insertReturn1).toEqual({
      id: 1,
      handle: USERS[0].handle,
    });

    const insertReturn2 = await idAndHandleFacet.insertOne(USERS[1]);
  ...
  it("allows for returning all columns", async () => {
    ...
    const insertReturn1 = await allColumnsFacet.insertOne(USERS[0]);
    expect(insertReturn1).toEqual({
      id: 1,
      handle: USERS[0].handle,
      name: USERS[0].name,
    ...
    const insertReturn2 = await allColumnsFacet.insertOne(USERS[1]);
    expect(insertReturn2).toEqual({
      id: 2,
      handle: USERS[1].handle,
      name: USERS[1].name,
